...
export default function Testimonials() {
  ...
  return (
    <section ref={sectionRef} className={`py-20 bg-sage/60 transition-opacity duration-700 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <div className="container-main">
        <div className="grid md:grid-cols-2 gap-12 items-center max-w-5xl mx-auto">
          ...
          <div className={`anim-fade-right anim-delay-2 ${isVisible ? 'is-visible' : ''}`}>
            ...
            <div className="flex gap-2">
              {testimonials.map((_, i) => (
                <button key={i} onClick={() => setCurrent(i)} className={`w-2 h-2 rounded-full border-none cursor-pointer transition-all duration-300 ${i === current ? "bg-olive w-5" : "bg-olive/30 hover:bg-olive/50"}`} aria-label={`Testimonial ${i + 1}`} />
              ))}
            </div>
          </div>
        </div>
